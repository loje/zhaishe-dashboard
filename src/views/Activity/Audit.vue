			...
			<div class="top-title">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					...
					<el-breadcrumb-item v-loading="loading">{{
						$route.query.title
					}} <el-tag type="info">当前共{{total}}人报名</el-tag></el-breadcrumb-item>
				</el-breadcrumb>
			</div>
			...
			<div class="top-func">
				...
				<!-- <div class="the-func">
					...
					<el-select size="small">
						<el-option>支付成功</el-option>
						<el-option>发生退款</el-option>
						<el-option>支付失败</el-option>
		...
		<div class="layer-table">
			...
			>
				...
				<el-table-column label="商户订单号（微信支付订单号）" width="220">
					<template slot-scope="scope">
						{{ scope.row.order.payReslut.out_trade_no }}
						<br />
						（{{ scope.row.order.payReslut.transaction_id }}）
					</template>
				</el-table-column>
				...
				<el-table-column label="微信" prop="wechatId" align="center">
					<template slot-scope="scope">{{
						scope.row.order.wechatId || scope.row.wechat.wechatId
					}}</template>
				</el-table-column>
				<el-table-column
					label="邮箱"
	...
	methods: {
		...
		getActivityData() {
			...
			activityData.get(this.$route.query.id).then((res) => {
				...
				this.pickerOptions = {
					disabledDate(time) {
						return time.getTime() < new Date(res.startTime.iso).getTime() || time.getTime() > new Date().getTime();
					},
				}
		...
		async getCanvasData() {
			if (this.$route.query.id) {
				...
				if (this.applicationDate.length === 0) {
					...
					diffNow = this.$moment(this.applicationDate[1]).diff(this.$moment(this.applicationDate[0]), 'days') + 1;
				}
				...
				if (this.applicationDate.length === 0) {
					if (diffNow > 7) {
						for (let i = 0; i < 7; i += 1) {
							...
							const query = this.$Bmob.Query("activity_person");
							query.equalTo("activity", "==", poiID);
							query.equalTo("createdAt", ">=", new Date(`${this.$moment().subtract(i, 'days').format('YYYY-MM-DD')} 00:00:00`));
							query.equalTo("createdAt", "<", new Date(`${this.$moment().subtract(i, 'days').format('YYYY-MM-DD')} 23:59:59`));
							let data = await query.count();
							dayList.push({ date: this.$moment().subtract(i, 'days').format('YYYY-MM-DD'), value: data })
						}
					} else {
					...
					for (let i = 0; i < diffNow; i += 1) {
						...
						const query = this.$Bmob.Query("activity_person");
						query.equalTo("activity", "==", poiID);
						query.equalTo("createdAt", ">=", new Date(`${this.$moment(this.applicationDate[1]).subtract(i, 'days').format('YYYY-MM-DD')} 00:00:00`));
						query.equalTo("createdAt", "<", new Date(`${this.$moment(this.applicationDate[1]).subtract(i, 'days').format('YYYY-MM-DD')} 23:59:59`));
						let data = await query.count();
						dayList.push({ date: this.$moment(this.applicationDate[1]).subtract(i, 'days').format('YYYY-MM-DD'), value: data })
					}
				}
